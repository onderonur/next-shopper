...
export function useSelectableItemProps() {
  return {
    ...
    itemClassName: twJoin(
      ...
      '[&+label]:enabled:cursor-pointer [&+label]:enabled:hover:bg-overlay-light [&+label]:enabled:active:bg-overlay',
    ),
    indicatorClassName: 'flex items-center justify-center text-white',
    icon: <CheckIcon className="mt-0.5" size="1.2rem" />,
    labelClassName:
      'w-full select-none rounded-md p-2 text-sm font-semibold text-foreground-light',
  };
}
